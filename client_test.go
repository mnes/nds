...
func TestClient_onError(t *testing.T) {
	...
	c, err := nds.NewClient(ctx, testCacher, time.Second, nds.WithDatastoreClient(dsClient))
	if err != nil {
	...
	c, err = nds.NewClient(ctx, testCacher, time.Second, nds.WithDatastoreClient(dsClient), nds.WithOnErrorFunc(onErrFn))
	if err != nil {
		t.Fatalf("could not make nds client due to error: %v", err)
	}
...
func TestNewClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := nds.NewClient(tt.args.ctx, tt.args.cacher, time.Second)
